...
def run_connectivity_checks(
    ...
    results = []
    # probe 始终使用实时标识（调用方传入或本地生成），不再使用固定 "probe"。
    # 这样 .xxx / .xxxx 占位符会被真实 sticky id 替换，与浏览器出口一致。
    ident = str(identifier or "").strip() or secrets.token_hex(8)
    placeholder = str(config.get("proxy_placeholder", ".xxx") or ".xxx")
    proxy = resolve_proxy_url(config.get("proxy", ""), ident, placeholder)
